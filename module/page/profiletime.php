<?php
	...
	if(!mysqli_num_rows($timephoto)){
		...
		$url="index.php?mod=page&do=profiletime";
		$limit=1;
	$frmdataget=$_REQUEST;
		PaginationWork();
$totRslt=mysqli_fetch_assoc(mysqli_query($con,"select count(*) as tot from photos left join tag on pid=photos.id left join profile on uid=profile.id $wh"));
		$query=mysqli_query($con,"select photos.id, profile.name as user,photos.uid,profile.propic,profile.gender, photos.name, time, description from photos left join tag on pid=photos.id left join profile on uid=profile.id where photos.uid=$uid or tag.fid=$uid group by photos.id order by photos.time desc limit ".$frmdata['from'].", ".$frmdata['to']);
		$i=1;
			while($timephoto=mysqli_fetch_assoc($query)){
			?>
            	<div class="photoshow">
            		<table width="100%" border="0px" cellspacing="5px" style="padding:5px 10px 5px 10px; margin-top:100px; margin-bottom:40px;">
                    	<tr>
            ...
            <?php }?>
                ...
                <td colspan="2"><a href="index.php?mod=profile&do=profile&id=<?php echo $timephoto['uid'];?>"><?php echo ucfirst($timephoto['user']);?></a></td>
            </tr>
            <tr>
            	<td colspan="2">
            ...
            <div align="center">
            <?php PaginationDisplay($totRslt['tot'],$url.'&pageNumber=','');?>
            </div>
	<?php ?>
<script>
